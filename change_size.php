...
session_start();  
   ...
   if(isset($_POST['addBtn'])){
      $size = $_POST['size'];
      $sizeid = $_POST['id'];
      $q = $conn->prepare("UPDATE sizes SET size = :size WHERE id = :id");
      $q->bindParam(':id',$sizeid,PDO::PARAM_STR);
      $q->bindParam(':size',$size,PDO::PARAM_STR);
      $q->execute();
  }

  $sql = 'SELECT id, `size` FROM sizes ORDER BY `size`';
  $q = $conn->query($sql);
...
<form action="change_size.php" method="POST">
    <p> 
        ...
        <input type="submit" value="change size" name="addBtn" />
    </p>
</form>
